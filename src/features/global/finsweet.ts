...
function ensureFinsweetGlobal(): any {
	if (typeof window === "undefined") {
		throw new Error(
			"FinsweetService can only run in a browser environment.",
		);
	}
...
export class FinsweetService {
	 ...
	 */
	public async getListByAttribute(
		attribute: string,
		...
		if (loaders.length) {
			try {
				...
				console.warn(
					"[FinsweetService] Target list loaders rejected",
					err,
				);
			}
		}
	...
	public clearFiltersExceptFor(
		...
		if (typeCondition) {
			const plainTypeCondition = JSON.parse(
				JSON.stringify(typeCondition),
			);
